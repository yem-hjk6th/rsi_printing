...
def _ftm_register(new_pcd, model_pcd, T_prev):
    """
    Register new_pcd (camera frame) against model_pcd (world frame) via:
      1. FPFH+RANSAC coarse (handles large angular jumps between discrete views)
      2. Geometric ICP medium refinement
      3. Colored ICP fine refinement
    Returns (T_world, fitness, rmse, ok).
    """
    try:
        # Step 1: RANSAC coarse against model (breaks rotational symmetry since
        #         model is multi-view fused — no single-view symmetry)
        s_d, s_f = _compute_fpfh(new_pcd, VOXEL_COARSE)
        m_d, m_f = _compute_fpfh(model_pcd, VOXEL_COARSE)
        T_coarse  = _ransac(s_d, m_d, s_f, m_f, VOXEL_COARSE)
        # Step 2: geometric ICP
...
def register_frame_to_model(pcds, depths, colors, intrinsic,
    ...
    for i in range(FTM_WARMUP_FRAMES, n):
        ...
        if not ok:
            print(f"    FTM fitness={fit:.3f} < {FTM_MIN_FITNESS} — RANSAC fallback")
            # Fallback: pairwise RANSAC against previous view
            # T_fb maps pcds[i-1] → pcds[i]; compose onto world pose of i-1
            # world_pose_i = world_pose_{i-1} @ inv(T_fb)
            T_fb, fit_fb, rmse_fb = pairwise(pcds[i-1], pcds[i])
            T = all_poses[-1] @ np.linalg.inv(T_fb)
            fit, rmse = fit_fb, rmse_fb
            print(f"    Fallback fit={fit:.4f}  rmse={rmse*1000:.2f}mm")
            log.append(f"{i} FTM-FALLBACK  {fit:.4f}  {rmse*1000:.3f}mm")
        else:
