...
print(f"\nEnvironment self-check — ffs_poisson_detail")
...
try:
    ...
    if config.FFS_WEIGHTS_DIR.exists():
        ...
        _fail(f"FFS weights not found: {config.FFS_WEIGHTS_DIR}",
              "set FFS_REPO_ROOT env var, or create Vision/ffs_poisson_detail/"
...
if _FAILS:
    ...
    print("   Full new-machine bootstrap chain is in log.md (Environment section).")
    sys.exit(1)
else:
    print("=> smoke test PASS — capture / qc / recon are good to go.")
    sys.exit(0)
